...
__sco.config = {
    ...
    'guid': '', // V2 guid
    'v1guid': '', // V1 guid, the api key for the client

    'triggers': ['exit', 'timeout'], // Set the triggers to use, use either load or exit and timeout together

    'status1': ["STATUSONE"], // Strings to be set in lower case, will be compared against the URL of the current page. Functions will be executed and must return either true, false or a number to use as the status
    'status2': ["STATUSTWO"],
    'status3': ["STATUSTHREE"],
    'status4': ["STATUSFOUR"],
...
__sco.translatetov1 = function (obj) {
    try {
        ...
        if (status == "3") {
            return 'c=' + thisObj.i1 + '&cc=&ca=0&e=&sfs=ordernumber^' + (typeof thisObj.s.ordernumber == "string" ? thisObj.s.ordernumber : typeof thisObj.s.ordernum == "string" ? thisObj.s.ordernum : "") + '&scs=' + __sco.support.screeninfo + '&b=' + thisObj.s.i + '&ua=' + __sco.hash(navigator.userAgent);
        }
        else {
